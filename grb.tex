...
\begin{abstract}
Este estudio presenta un análisis espectral y temporal de los destellos de rayos
gamma bn090926181 y bn090424592, utilizando datos de los detectores NaI y BGO
del Fermi GBM.  Se generan curvas de luz en los rangos 8-50 keV, 50-300 keV y
300-900 keV y se realiza un ajuste del pico de emisión con el modelo de Band,
complementado por componentes de cuerpo negro, modelo comptonizado y ley de
potencias.  
Se incluye un análisis resuelto en el tiempo que muestra la evolución de los
parámetros espectrales y su correlación con la curva de luz del evento.  
Los resultados indican que la energía pico y el índice $\alpha$ siguen la forma
de la curva de luz, mientras que $\beta$ presenta mayor incertidumbre en
intervalos muy cortos; la combinación multicomponente mejora la descripción del
espectro observado. 
\end{abstract}
...
El modelo de cuerpo negro describe la emisión puramente térmica de un objeto en equilibrio. Su espectro está determinado únicamente por su temperatura y se expresa mediante la ley de Planck \cite{GDT-Core}:
\begin{equation}
...
Este modelo, también conocido como ley de potencias con un corte exponencial (\textit{cutoff power-law}), es a menudo utilizado para describir procesos de Comptonización, donde fotones de baja energía ganan energía a través de colisiones con una población de electrones calientes. La función se define como \cite{GDT-Core}:
\begin{equation}
...
La ley de potencias es el modelo más simple para describir la radiación no térmica, como la emitida por procesos de sincrotrón. Su forma matemática es \cite{GDT-Core}:
\begin{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{imagenes/lightcurve_n7_bn090424592.pdf}
    \caption{Curvas de luz del detector n7 para bn090424592 a tres rangos de energía.}
    \label{fig:lightcurve_n7_bn090424592}
\end{figure}
...
\begin{figure}
    ...
    \caption{Ajuste espectral para el intervalo de tiempo donde el conteo fue pico para el objeto bn090424592. \textbf{Panel superior:} Muestra el espectro de energía en unidades de tasa de cuentas (puntos con barras de error) para los detectores NaI (n7, n8, nb) y BGO (b1). La línea continua sobrepuesta a los datos representa el mejor ajuste obtenido con el modelo [Nombre del Modelo, e.g., Band].
        \textbf{Panel inferior:} Presenta los residuales del ajuste, calculados como la diferencia entre los datos observados y el modelo, en unidades de desviación estándar ($\sigma$).}
    \label{fig:fit_bnbn090424592_band}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{imagenes/espectro_vFv_bnbn090424592_band.pdf}
    \includegraphics[width=0.7\linewidth]{imagenes/espectro_vFv_bnbn090424592_multiple.pdf}
...
\begin{figure}
    ...
    \caption{Evolución temporal de los parámetros para el ajuste de tipo Band del objeto bn090926181. Se emplearon bines de 2.0 segundos.}
    \label{fig:181-band}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{imagenes/grafica_band_592_0.5.png}
    \caption{Evolución temporal de los parámetros para el ajuste de tipo Band del objeto bn090424592. Se emplearon bines de 0.5 segundos.}
    \label{fig:592-band}
\end{figure}

En la figura \ref{fig:lightcurve_n7_bn090424592} pueden apreciarse los cambios en la estructura de la curva de luz cuando se grafica a varios rangos de energía. En la figura \ref{fig:fit_bnbn090424592_band} se muestra el ajuste del modelo de Band para el objeto estudiado. La distribución de los residuales en torno a cero indica que el modelo describe adecuadamente los datos en todo el rango de energía analizado.

En la figura \ref{fig:espectro_vFv} se muestran los espectros en formato $\nu F_\nu$ para los dos tipos de ajuste. En el recuadro inferior se ve que el modelo de PowerLaw no produce resultados útiles. Esto puede confirmarse con la figura \ref{fig:592-multi}, en donde en el último cuadro, se ve que las incertezas del índice de PowerLaw impiden hacer predicciones de valor.

La Figura \ref{fig:181-band} muestra la evolución temporal de los parámetros de encaje de Band para el objeto bn090926181, los conteos por segundo (gris) y los ajustes para los parámetros de Energía pico (azul), $\alpha$ (verde) y $\beta$ (rojo) del objeto bn090926181, y lo mismo vemos representado en la figura \ref{fig:592-band} para el encaje de Band para el objeto bn090424592. En ambos casos podemos observar que la energía y el parámetro alpha tienden a seguir la forma general de la curva de luz mientras que el parámetro beta tiene un comportamiento más errático, mostrando barras de error mucho más grandes cuando el tamaño del bin es muy pequeño, pero que se puede mejorar utilizando un tamaño de bin mayor al costo de hacer menos evidente la correlación entre los parámetros de energía y alpha con la curva de luz. Esto se puede observar al notar como beta tiene un mejor comportamiento en la figura \ref{fig:181-band} donde se utilizó un bin de $2$ s contra los $0.5$ s en todos los otros análisis.

Si bien el modelo físico de múltiples componentes describe adecuadamente el espectro del GRB bn090926181, como se ilustra en la Figura \ref{fig:181-multi}, la aplicación de este mismo modelo al GRB bn090424592 no arrojó un resultado de calidad comparable. La principal dificultad residió en la componente de ley de potencias (PowerLaw), la cual no pudo ser constreñida de forma robusta por el ajuste. Esta falta de convergencia se manifiesta en la alta incertidumbre de sus parámetros, tal como se evidencia en la distribución posterior correspondiente en la figura de análisis de resultados.
...
En las figuras \ref{fig:181-multi} y \ref{fig:592-multi} representamos el modelo multicomponente, de nuevo mostrando la curva de luz de los eventos y luego la evolución espectral de los parámetros, en este caso, se puede observar que la energía del componente Comptonized y la temperatura del componente BlackBody se mueven reflejando la curva de luz del evento, mientras el índice fotónico del componente PowerLaw muestra un comportamiento más difícil de predecir y de darle provecho.
